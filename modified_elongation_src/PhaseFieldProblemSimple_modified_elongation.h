...
public:
  ...
  void MultiCellGrowDivide(AdaptInfo *adaptInfo){
    ...
    if(numInitialCells_ > 1){
      ...
      if(rank_<numInitialCells_ && rank_ >0){
        ...
        position[0] = domainDimension_[0]*(0.5 + 0.3*std::cos(orientation_ * M_PI / 180.0));
        position[1] = domainDimension_[1]*(0.5 + 0.3*std::sin(orientation_ * M_PI / 180.0));
  ...
  virtual void initTimestep(AdaptInfo *adaptInfo) override
  {
    ...
    //std::cerr << "Time Phasefield 1: " << elapsed1.count() << "\n";
    ...
    if(adaptInfo->getTimestepNumber() > 500 && volume_ > volume_threshold_*0.5)
    {
    ...
    //std::cerr << "Time Phasefield 2: " << elapsed2.count() << "\n";
    ...
    *getProblem()->getOldSolution() << valueOf(getProblem()->getSolution(0));
    ...
    if (confined_==1){
      ...
      *confinementForce_ << B1_w(getProblem()->getOldSolution(),confinementPF_.get());
      ...
      if(adaptInfo->getTimestepNumber()%50000 == 0){
    ...
    //std::cerr << "Time Phasefield 3: " << elapsed3.count() << "\n";
  ...
  virtual void closeTimestep(AdaptInfo *adaptInfo) override
  {
    ...
    //std::cerr << "Time Phasefield 4: " << elapsed4.count() << "\n";
