...
public:
  ...
  virtual void initTimestep(AdaptInfo *adaptInfo) override
  {
    ...
    //Now, we determine the head of the cell from two possible head direction:
    if(std::abs(major_axis_angle_rad_ - theta_) > M_PI/2){
        major_axis_angle_rad_ = -major_axis_angle_rad_;
    }
    theta_ += (major_axis_angle_rad_ - theta_)*angle_relaxation_coeff;
    //
    int qwerty = 1;
    std::random_device rd;
    std::mt19937 generator(rd()); 
    ...
    *advection_[1] << v0_ * std::sin(theta_) * (0.5*valueOf(getProblem()->getSolution(0)) + 0.5); //uncomment this
    ...
    if(elongation_vel){ 
      ...
      *advection_[0] << function_(elongateVelocity(getPosition(), v0_, theta_+(M_PI/2), 0.25), X())* std::cos(theta_) * (0.5*valueOf(getProblem()->getSolution(0)) + 0.5);
      *advection_[1] << function_(elongateVelocity(getPosition(), v0_, theta_+(M_PI/2), 0.25), X())* std::sin(theta_) * (0.5*valueOf(getProblem()->getSolution(0)) + 0.5);
    }
