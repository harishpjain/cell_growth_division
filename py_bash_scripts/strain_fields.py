...
def compute(folder):
    ...
    print(times)
    file_pattern = input_dir + '/v0_glob_*.npy'
    ...
    for time in times[1:]:
        ...
        v0_glob = np.load(input_dir + '/v0_glob_' + '{:06.3f}'.format(time) +'.npy')
        v1_glob = np.load(input_dir + '/v1_glob_' + '{:06.3f}'.format(time) +'.npy')
        grid_X = np.load(input_dir + '/grid_x_' + '{:06.3f}'.format(time) +'.npy')
        grid_Y = np.load(input_dir + '/grid_y_' + '{:06.3f}'.format(time) +'.npy')
    
        ax.contourf(grid_X,grid_Y,(v0_glob),alpha=0.5)
        # Smooth
        v0_glob = gaussian_filter(v0_glob, 10.0, mode=mode)
    
        # Smooth
        v1_glob = gaussian_filter(v1_glob, 10.0, mode=mode)
    
        print('filtered time ' + str(time))
        steps_x,steps_y = np.shape(v0_glob)
        
        grid_x_vec = np.reshape(grid_X, (steps_x*steps_y,1))
        grid_y_vec = np.reshape(grid_Y, (steps_x*steps_y,1))
        
        v0_interp = griddata((np.squeeze(grid_x_vec),np.squeeze(grid_y_vec)),np.reshape(v0_glob,(steps_x*steps_y,1)),(xx,yy))
        v1_interp = griddata((np.squeeze(grid_x_vec),np.squeeze(grid_y_vec)),np.reshape(v1_glob,(steps_x*steps_y,1)),(xx,yy))
    
        # phase angle
        omega = np.multiply(np.sign(v1_interp),(np.arctan(v0_interp/np.abs(v1_interp))/ 2.0 + np.pi/4.0))
        ...
        #finding strain fields
        grid_spacing = grid_x_vec[1] - grid_x_vec[0]
        print(len(grid_spacing))
        grad_v0 =  np.array(np.gradient(v0_glob))/grid_spacing
        grad_v1 =  np.array(np.gradient(v1_glob))/grid_spacing
        grad_v = np.stack(grad_v0, grad_v1)
        print(v0_glob.shape)
        print((grad_v).shape)

        """
        # Identify 0 contours of S0 and S1
        c1 = ax.contour(grid_X,grid_Y,v0_glob,0,colors='red',alpha=0.5)
        c2 = ax.contour(grid_X,grid_Y,v1_glob,0,colors='blue',alpha=0.5)
        ...
        """
        fig.savefig(result_dir+'/velocity_'+str(count).zfill(4)+'.png', dpi=300, bbox_inches='tight') 
        plt.close(fig)
        count += 1
