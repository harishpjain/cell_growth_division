...
public:
  ...
  virtual void fillOperators(ProblemType *prob) override
  {
	...
	*/
    ...
    prob->addMatrixOperator(opPhi2, 1, 1);  //G(phi)*mu
    ...
    //Surface tension killer terms from Aland et al
    for (int i = 0; i < dow_; ++i) {
      for (int j = 0; j < dow_; ++i) {
        Operator *opAland = new Operator(prob->getFeSpace(1), prob->getFeSpace(0));
        addSOT(opAland, derivativeOf(prob->getSolution(0), i)*derivativeOf(prob->getSolution(0), j));
        prob->addVectorOperator(opAland, 1, 0);
      }
    }
